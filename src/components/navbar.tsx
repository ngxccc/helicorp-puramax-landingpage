          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <Popover>
              <PopoverTrigger asChild>
                ...
                >
                  <svg
                    className={`h-4.5 w-4.5 ${favorites.length > 0 ? "fill-red-500 stroke-red-500 text-red-500" : "fill-none stroke-currentColor"}`}
                    viewBox="0 0 24 24"
                    strokeWidth="2"
                  >
            ...
            <Popover>
              ...
              >
                <h4 className="mb-3 text-sm font-bold text-slate-900 dark:text-white">
                  Giỏ hàng ({cart.reduce((sum, item) => sum + (item.quantity ?? 1), 0)})
                </h4>
                {cart.length === 0 ? (
                  <p className="py-4 text-center text-xs text-slate-500 dark:text-zinc-400">
                    Giỏ hàng trống.
